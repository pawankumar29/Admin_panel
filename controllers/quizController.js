      ...
      },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            { $project: { _id: 1, name: 1, no_of_students: 1 } },
          ],
          as: "institute",
        },
      },
...
exports.getQuizDetail = async (req, res, next) => {
  try {
    ...
    const quizQuestions = await questions.find({
      ...
      sub_category_id: req.params.sub_cat_id,
    });
    if (quizQuestions != null) {
      quizQuestions.start_time = momenttz
        .tz(quizQuestions.start_time, req.cookies.time_zone_offset)
  ...
  new Promise((resolve, reject) => {
    quizzes.findOne({ _id: req.params.id }).then((data) => {
      if (data != null) {
        ...
        data.start_time = momenttz
          .tz(data.start_time, req.cookies.time_zone_offset)
          .format("YYYY-MM-DD HH:mm");
    ...
    question_categories
      ...
      .then((data) => {
        ...
        for (var i = 0; i < data.sub_category.length; i++) {
          if (
            data.sub_category[i]._id.toString() ==
            req.params.sub_cat_id.toString()
          ) {
          ...
          for (let index = 0; index < csvDataArray.length; index++) {
            ...
            if (
              ...
              if (answer.length) {
                ...
                if (sub_cat_id != "") obj["sub_category_id"] = sub_cat_id;
                validArray.push(obj);
              } else {
