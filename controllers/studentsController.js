        ...
        pipeline: [
          {
            $match: {
              $expr: {
                $eq: ["$name", new RegExp(".*" + req.query.name + ".*", "i")],
              },
            },
          },
        ],
...
exports.student_shortlist = (req, res, next) => {
  try {
    //shortlistIds
    const shortlistIds = [];
    //fetching ids from ajax
    if (req.body.data) {
      req.body.data.forEach((element) => {
        shortlistIds.push(mongoose.Types.ObjectId(element));
      });
    }
    //update data
    quiz_results
      .updateMany({ _id: { $in: shortlistIds } }, { student_shortlist: 1 })
      .then((updatedData) => {
        if (updatedData != null) {
...
exports.send_offerletter = (req, res, next) => {
  try {
     //offerLetterIds
     const offerLetterIds = [];
     //fetching ids from ajax
     if (req.body.data) {
       req.body.data.forEach((element) => {
        offerLetterIds.push(mongoose.Types.ObjectId(element));
       });
     }
    //update data
    quiz_results
      .updateMany({ _id: { $in: offerLetterIds } }, { offer_letter_issued: 1 })
      .then((updatedData) => {
        if (updatedData != null) {
          return res.send( { status: 1,data:updatedData})
        } else {
          return res.send({ status: 0 });
        }
      })
