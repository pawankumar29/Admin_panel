...
jQuery(document).ready(function () {
  ...
  $(".forgot-form").validate({
    ...
    rules: {
      email: {
        ...
        maxlength: 50,
        match:
          /^[a-z0-9_\-]+(\.[_a-z0-9\-]+)*@([_a-z0-9\-]+\.)+([a-z]{2}|biz|com|edu|gov|info|net|org)$/,
      },
    },
    messages: {
      email: {
  ...
  $(".changePassword-form").validate({
    ...
    messages: {
      ...
      newPassword: {
        ...
        rangelength:
          "PLease enter a new password between 8 to 16 characters. Your password should include letters, numbers and special characters.",
        match:
          "Enter a password between 8 to 16 characters. Your password should include letters, numbers and special characters.",
      },
      confirmPassword: {
  ...
  $(document).on("click", "#checkAll", function () {
    ...
    if ($(this).prop("checked")) $(".check").parent().prop("class", "checked");
    else $(".check").parent().prop("class", "");
  });
  ...
  $(document).on("click", ".custom_close", function () {
    ...
    if ($(".custom_close").length == 0) {
      $(".chipcontainer").html(
        "<h1 style='color:red'><b>PLease select atleast one college</b></h1>"
      );
    }
  ...
  $("#enable_test").click(function () {
    ...
    if ($(".check:checked").length == 0) {
      ...
      checkedData.forEach(function (obj) {
        text =
          text +
          "<span class='chip' data-value='" +
          obj.value +
          "'>" +
          obj.name +
          "<span class='custom_close'><i class='fa fa-remove' aria-hidden='true'></i></span></span>";
      });
...
$(document).on("click", ".delete", function () {
  console.log("ajaxxxxxxxxxxxxxxxxxxxxxxxx");
  var message = "Are you sure you want to delete?";
  if ($("#url").val() === "/institutes/") {
    message = "Are you sure you want to delete this Institution?";
  }
  if ($("#url").val() === "/test/") {
    message = "Are you sure you want to delete a test?";
  }
...
$(document).on("click", ".change_status", function () {
  ...
  else msg = "Are you sure you want to activate this " + user_type + "?";
...
$(document).on("click", "#filter", function () {
  ...
  if (flag === 1) {
    ...
    $.ajax({
      ...
      data: { search: 1 },
      success: function (result) {
        if (result == "unauthorised") window.location = "/login";
        else $("#ajaxResponce").html(result);
  ...
  $.ajax({
    ...
    success: function (result) {
      if (result == "unauthorised") window.location = "/login";
        ...
        if (
          result.userDetailData.profile_pic == "no-image" ||
          result.userDetailData.profile_pic == null ||
          result.userDetailData.profile_pic == ""
        ) {
          user
            .find(".profile_pic .single_image")
            .attr("href", "/images/default.png");
          user
            .find(".profile_pic .img-circle")
            .attr("src", "/images/default.png");
        } else {
          user
            .find(".profile_pic .single_image")
            .attr("href", result.userDetailData.profile_pic);
          user
            .find(".profile_pic .img-circle")
            .attr("src", result.userDetailData.profile_pic);
          user
            .find(".profile_pic .img-circle")
            .attr(
              "alt",
              result.userDetailData.firstName + result.userDetailData.lastName
            );
        }
    ...
    success: function (result) {
      if (result == "unauthorised") window.location = "/login";
        ...
        query
          .find(".username")
          .text(
            result.query_data.data[0].user.firstName +
              " " +
              result.query_data.data[0].user.lastName
          );
        ...
        if (result.query_data.data[0].status == 0) {
          let create_resolve_link =
            ` <div class="resolved"> <a id='my_anchor' linkdata='/contact_us/update/` +
            id +
            `'><b>Mark as resolved</b></a>
